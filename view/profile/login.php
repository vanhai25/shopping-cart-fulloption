...
if(isset($_POST['sm'])){
    ...
    $check = $model->checkNameShop($name);
    if(strlen($name) < 6){
    ...
    elseif($check){
        ...
        $a = $model->insertShop($_SESSION['iduser'],$name,$nameKo);
        if($a){
            $_SESSION['idshop'] = $model->getIdShop($_SESSION['iduser'])->id;
